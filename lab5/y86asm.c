...
void take_value(bin_t *y86bin,int entry,int value)
{
	y86bin->codes[entry] = value & 0xFF;
	y86bin->codes[entry+1] = (value >> 8) & 0xFF;
	y86bin->codes[entry+2] = (value >> 16) & 0xFF;
	y86bin->codes[entry+3] = (value >> 24) & 0xFF;
}
...
parse_t parse_symbol(char **ptr, char **name)
{
	...
	int len=0;
	while (IS_LETTER(cur+len) || IS_DIGIT(cur+len)) len++;
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
	...
	if (IS_LETTER(cur)) return parse_symbol(ptr,name);
...
parse_t parse_data(char **ptr, char **name, long *value)
{
	...
	if(IS_DIGIT(cur)) return parse_digit(ptr,value);
    /* if IS_LETTER, then parse the symbol */
	if(IS_LETTER(cur)) return parse_symbol(ptr,name);
...
{
	/* skip the blank and check */
	char *loc,*check,*n=*name;
	SKIP_BLANK(*ptr);
	loc = strchr(*ptr,':');
	if (loc != NULL) {
		check = strchr(*ptr,'#');
		if(check!=NULL && check < loc)
			return PARSE_ERR;
	/* allocate name and copy to it */
		size_t len = loc-*ptr;
 ...
 */
#define judge(p,name) if (p==-1) {err_print(name); line->type=TYPE_ERR; goto out;}
...
cont:
    ...
    switch (HIGH(inst->code)) {
      ...
      case I_POPL: {/* B:0 regA:F - e.g., popl %ebp */
        /* parse register */
	judge(parse_reg(&cur,&regA),"Invalid REG");
        /* set y86bin codes */
	...
	judge(parse_reg(&cur,&regA),"Invalid REG");
	judge(parse_delim(&cur,','),"Invalid ','");
	judge(parse_reg(&cur,&regB),"Invalid REG");
	...
	judge(ret,"Invalid Immediate");
	judge(parse_delim(&cur,','),"Invalid ','");
	judge(parse_reg(&cur,&regB),"Invalid REG");

	y86bin->codes[1] = HPACK(0xF,regB);
	if (ret == PARSE_DIGIT) take_value(y86bin,2,lval); else
	if (ret == PARSE_SYMBOL) add_reloc(symbol,y86bin,2);
        goto cont;
      }
	...
	judge(parse_reg(&cur,&regA),"Invalid REG");
	judge(parse_delim(&cur,','),"Invalid ','");
	judge(parse_mem(&cur,&lval,&regB),"Invalid MEM");
	y86bin->codes[1] = HPACK(regA,regB);
	take_value(y86bin,2,lval);
	goto cont;
      }
	...
	char *name;
	judge(parse_symbol(&cur,&name),"Invalid DEST");
	add_reloc(name,y86bin,1);
        goto cont;
      }
      ...
      case I_DIRECTIVE: {
		...
		if (parse_data(&cur,&symbol,&lval) == PARSE_SYMBOL) 
			add_reloc(symbol,y86bin,0);
		else 
			take_value(y86bin,0,lval);
		goto cont;
          }
          
          case D_POS: {   /* .pos D - e.g., .pos 0x100 */
		long pos;
		judge(parse_digit(&cur,&pos),"Invalid digit");
		vmaddr = pos;
		y86bin->addr = pos;
            goto cont;
          }
		...
		long align;
		judge(parse_digit(&cur,&align),"Invalid digit");
		if(vmaddr % align != 0) {
...
int relocate(void)
{
	...
	take_value(rtmp->y86bin,rtmp->entry,symbol->addr);
        rtmp = rtmp->next;
    }
    return 0;
}
...
int binfile(FILE *out)
{
	...
	line_t *p;
	char *buf = (char*)malloc(MAX_INSLEN * 6);
	char *buf_beg = buf;
	for (i=0;i<MAX_INSLEN * 6;i++) buf[i]=0;
	long pos;
	for (p = y86bin_listhead->next; p; p=p->next)
	{
		buf = buf_beg + p->y86bin.addr;
		memcpy(buf, p->y86bin.codes, p->y86bin.bytes);
		if (p->y86bin.bytes!=0)
		  pos = p->y86bin.addr+p->y86bin.bytes;
	}
